...
- CI base isolation and stress watchdogs: the base regression GREEN passed 4/4; the watchdog RED lacked both bounded declarations and GREEN passed 5/5 while preserving the unset global timeout. The third GitHub run proved the local 120s/60s limits are insufficient in CI, so only the two bounded stress tests use `process.env.CI` limits of 300s/180s and local limits of 120s/60s. Two isolated `CI=true GITHUB_ACTIONS=true` integrations passed 24 tests with 8 intended skips (44.1 s and 42.7 s), including direct routes, keyboard, phase-marker output, stress cleanup, and unchanged sharing-module timeout behavior.
