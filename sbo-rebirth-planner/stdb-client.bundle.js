...
${originalIndentation}`;
  ...
  // stdb-client-entry.ts
  var stdb_client_entry_exports = {};
  __export(stdb_client_entry_exports, {
    connect: () => connect,
    deleteRemoteDoc: () => deleteRemoteDoc,
    disconnect: () => disconnect,
    getRemoteDocJson: () => getRemoteDocJson,
    isConnected: () => isConnected,
    subscribe: () => subscribe,
    upsertRemoteDoc: () => upsertRemoteDoc
  });
...
${ty.variants.map(
  ...
  // stdb-bindings/delete_planner_doc_reducer.ts
  var delete_planner_doc_reducer_default = {
    docKey: t.string()
  };

  // stdb-bindings/upsert_planner_doc_reducer.ts
  var upsert_planner_doc_reducer_default = {
    docKey: t.string(),
    docJson: t.string()
  };

  // stdb-bindings/my_planner_docs_table.ts
  var my_planner_docs_table_default = t.row({
    id: t.u64(),
    owner: t.identity(),
    docKey: t.string().name("doc_key"),
    docJson: t.string().name("doc_json"),
    updatedAt: t.timestamp().name("updated_at")
  });

  // stdb-bindings/index.ts
  var tablesSchema = schema({
    my_planner_docs: table({
      name: "my_planner_docs",
      indexes: [],
      constraints: []
    }, my_planner_docs_table_default)
  });
  var reducersSchema = reducers(
    reducerSchema("delete_planner_doc", delete_planner_doc_reducer_default),
    reducerSchema("upsert_planner_doc", upsert_planner_doc_reducer_default)
  );
  ...
  function readAllDocs(c) {
    const map = /* @__PURE__ */ new Map();
    const table2 = c.db.my_planner_docs;
    if (!table2 || typeof table2.iter !== "function") return map;
  ...
  function wireTable(c) {
    ...
    table2.onInsert(() => bump());
    table2.onDelete(() => bump());
    if (typeof table2.onUpdate === "function") {
      table2.onUpdate(() => bump());
    }
  }
  ...
  function getRemoteDocJson(docKey) {
    if (!conn) return null;
  ...
  function connect(cfg, onStatus) {
    disconnect();
    tokenStorageKey = tokenKey(cfg);
    const existing = typeof localStorage !== "undefined" ? localStorage.getItem(tokenStorageKey) : null;
    const builder = DbConnection.builder().withUri(cfg.uri).withDatabaseName(cfg.databaseName).withToken(existing || void 0).onConnect((c, _identity, token) => {
      try {
  ...
  function upsertRemoteDoc(docKey, docJson) {
    pendingWrites.set(docKey, docJson);
    if (conn) {
      flushPending(conn);
    }
  }
  function deleteRemoteDoc(docKey) {
  ...
  return __toCommonJS(stdb_client_entry_exports);
})();
